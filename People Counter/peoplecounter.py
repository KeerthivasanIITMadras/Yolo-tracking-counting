...
while True:
    ...
    for results in results_Tracker:
        ...
        cvzone.putTextRect(img, f"{Id}", (max(0, x1), max(35, y1 - 20)), scale=2,
                           thickness=1, offset=3)
        if limitsDown[1] + 10 > cy > limitsDown[1] - 10 and limitsDown[0] < cx < limitsDown[2]:
            if totalCountDown.count(Id) == 0:
                totalCountDown.append(Id)
                cv2.line(img, (limitsDown[0], limitsDown[1]), (limitsDown[2], limitsDown[3]), (0, 255, 0), 5)
        if limitsUp[1] + 10 > cy > limitsUp[1] - 10 and limitsUp[0] < cx < limitsDown[2]:
            if totalCountUp.count(Id) == 0:
                totalCountUp.append(Id)
                cv2.line(img, (limitsUp[0], limitsUp[1]), (limitsUp[2], limitsUp[3]), (0, 255, 0), 5)

    # cvzone.putTextRect(img, f"Counts:{len(total_counts)}", (50, 50))
    cv2.putText(img, str(len(totalCountDown)), (1191, 345), cv2.FONT_HERSHEY_PLAIN, 5, (50, 50, 255), 8)
    cv2.putText(img, str(len(totalCountUp)), (929, 345), cv2.FONT_HERSHEY_PLAIN, 5, (50, 50, 255), 8)
    cv2.waitKey(1)
